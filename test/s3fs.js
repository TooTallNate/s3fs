...
//                    });
        ...
        it("should be able to copy a directory recursively with a promise", function (done) {
            Q.all(s3fsImpl.writeFile('testCopyDirPromise/test.json', '{}'), s3fsImpl.writeFile('testCopyDirPromise/test/test.json', '{}')).then(function () {
                s3fsImpl.copyDirectory('testCopyDirPromise', 'testCopyDirDestPromise').then(function () {
                    s3fsImpl.readdir('testCopyDirDestPromise').then(function (files) {
                        try {
                            files.should.be.an.instanceof(Array);
                            files.length.should.be.exactly(2);
                            files[0].should.be.exactly('test.json');
        ...
        it("should be able to copy a directory recursively with a callback", function (done) {
            Q.all(s3fsImpl.writeFile('testCopyDirCb/test.json', '{}'), s3fsImpl.writeFile('testCopyDirCb/test/test.json', '{}')).then(function () {
                s3fsImpl.copyDirectory('testCopyDirCb', 'testCopyDirDestCb', function (err) {
                    if (err) {
                        return done(err);
                    }

                    s3fsImpl.readdir('testCopyDirDestCb').then(function (files) {
                        try {
                            files.should.be.an.instanceof(Array);
                            files.length.should.be.exactly(2);
                            files[0].should.be.exactly('test.json');
        ...
        it("should be able to delete a directory recursively with a promise", function (done) {
            Q.all(s3fsImpl.writeFile('testDeleteDirPromise/test.json', '{}'), s3fsImpl.writeFile('testDeleteDirPromise/test/test.json', '{}'), s3fsImpl.writeFile('testDeleteDirPromise/test/test/test.json', '{}')).then(function () {
                s3fsImpl.rmdirp('testDeleteDirPromise/test').then(function () {
                    s3fsImpl.readdir('testDeleteDirPromise').then(function (files) {
                        try {
                            files.should.be.an.instanceof(Array);
                            files.length.should.be.exactly(1);
                            files[0].should.be.exactly('test.json');
                            done();
                        } catch (err) {
                            done(err);
        ...
        it("should be able to delete a directory recursively with a callback", function (done) {
            Q.all(s3fsImpl.writeFile('testDeleteDirCb/test.json', '{}'), s3fsImpl.writeFile('testDeleteDirCb/test/test.json', '{}')).then(function () {
                s3fsImpl.rmdirp('testDeleteDirCb/test', function (err) {
                    if (err) {
                        return done(err);
                    }

                    s3fsImpl.readdir('testDeleteDirCb').then(function (files) {
                        try {
                            files.should.be.an.instanceof(Array);
                            files.length.should.be.exactly(1);
                            files[0].should.be.exactly('test.json');
        ...
        it("should list all the files in a directory recursively with promise", function (done) {
            Q.all(s3fsImpl.writeFile('testCopyDirPromise/test.json', '{}'), s3fsImpl.writeFile('testCopyDirPromise/test/test.json', '{}')).then(function () {
                s3fsImpl.readdirp('testCopyDirPromise').then(function (files) {
                    try {
                        files.should.be.an.instanceof(Array);
